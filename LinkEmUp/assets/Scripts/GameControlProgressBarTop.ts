...
import { InitialMap, PointWidth, PointHeight, PointGap, InitiaRowCount, InitiaColCount, GameAreaWidth, GameAreaHeight, PointCategory, PointType } from './GameConfig';
...
export default class Game extends cc.Component {
  ...
  @property({ type: EffectCtrlProgressBarTop, tooltip: '控制完成弹窗、金钱增加的脚本' })
  EffectCtrl: EffectCtrlProgressBarTop = null;
  ...
  async onTouchEnd(event: cc.Event.EventTouch) {
    if (this.SelectedPointNodes.length >= 2) {
      ...
      await this.InspectAndEliminate();
      this.Reset();
  ...
  ApplyJoin(PointNode: cc.Node) {
    ...
    // 连线的PointNode是不是特殊PointNode类型或者当前连线的类型
    if (!this.IsRwardsType(Point.type) && this.LineType !== Point.type) return;
  ...
  /** 检查并消除 */
  async InspectAndEliminate() {
    /** 消除动画的Promise集合 */
    const RemovePromises = [];
    /** 奖励消除的节点集合 */
    const RwardSelectedPointNodes = this.Inspect();
    const Eliminate = [...RwardSelectedPointNodes, ...this.SelectedPointNodes];
    // 去重后逐一消除
    Array.from(new Set(Eliminate)).forEach((PointNode, PointId) => {
  ...
  /** 检查 */
  Inspect() {
    /** 所有已连接的PointNode中是否存在特殊类型 */
    const RwardsNodes = this.SelectedPointNodes.filter(PointNode => this.IsRwardsType(PointNode.getComponent(PointControl).type));
    /** 匹配的PointNode数组 */
    const matchNodes: cc.Node[] = [];
    RwardsNodes.forEach(PointNode => {
      const Point = PointNode.getComponent(PointControl);
      const { row, col } = Point;
      const RwardType = PointType.get(Point.type).rwardType;
      /** 增量 */
      let inc = 1;

      if (RwardType.includes('horizontal')) {
        ...
        while (InRange(col - inc, 0, InitiaColCount - 1) || InRange(col + inc, 0, InitiaColCount - 1)) {
          ...
          if (InRange(col - inc, 0, InitiaColCount - 1)) {
            matchNodes.push(this.PointNodes[row][col - inc]);
          }
          // 同上
          if (InRange(col + inc, 0, InitiaColCount - 1)) {
            matchNodes.push(this.PointNodes[row][col + inc]);
      ...
      rowPointNodes.forEach((PointNode, col) => {
        ...
        if (PointNode !== null) {
          ...
          if (Math.random() < 0.95) {
            ...
            type = Math.floor(Math.random() * 3 + 6);
          }
          GeneratePointNodes[row][col] = type;
        }
      });
    });
  ...
  GetPointPos(row, col): cc.Vec2 {
    const pos = this.node.convertToNodeSpaceAR(this.node.parent.convertToWorldSpaceAR(this.node.position));
    const BeginX = pos.x - this.node.width / 2 + PointWidth / 2;
    const BeginY = pos.y + this.node.height / 2 - PointHeight / 2;
  ...
  /** 判断类型是否是特殊类型 */
  IsRwardsType(type: number) {
    return PointType.has(type) && PointType.get(type).category === PointCategory.Rwards;
  }
}
