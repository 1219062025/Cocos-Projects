...
export default class PointControl extends cc.Component {
  ...
  @property({ type: cc.Prefab, tooltip: '消除时粒子特效预制体' })
  DieParticlePrefab: cc.Prefab = null;
  ...
  async Init(type: number, row: number, col: number, position: cc.Vec2, parent: cc.Node) {
    ...
    cc.loader.loadRes(`LinkEmUp/point/${PointType.get(this.type).label}Item`, cc.SpriteFrame, (err, res) => {
      if (err) return;
