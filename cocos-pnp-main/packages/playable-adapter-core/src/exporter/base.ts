...
const globalReplacer = (
  options: Pick<TBuilderOptions, "channel" | "resMapper"> & { $: CheerioAPI }
) => {
...
const compressScripts = (
  $: CheerioAPI,
  payload: { resMapper: TBuilderOptions["resMapper"] }
) => {
  ...
  for (let index = 0; index < splitCount; index++) {
    ...
    if (index === 0) {
      $(`script[data-id="adapter-zip-0"]`).html(
        `window.__adapter_zip__="${str}";`
      );
    } else {
      $(`script[data-id="adapter-zip-${index - 1}"]`).after(
        `<script data-id="adapter-zip-${index}">window.__adapter_zip__+="${str}";</script>`
      );
    }
  }
...
const restoreScripts = (
  ...
  const appendScript = (content: TResourceData, index: number) => {
    ...
    if (index === 0) {
      $(`script[data-id="adapter-resource-0"]`).html(
        `window.__adapter_resource__=${str};`
      );
      return;
    }
...
export const exportDirZipFromSingleFile = async (
  singleFilePath: string,
  options: TZipFromSingleFileOptions
) => {
  const {
    channel,
    transformHTML,
  ...
  for (let index = 0; index < scriptNodes.length; index++) {
    ...
    scriptNode.replaceWith(
      `<script src=".${jsDirname}/${jsFileName}"></script>`
    );
