const { ccclass, property, requireComponent } = cc._decorator;

/**
 * 数字滚动（cc.Label）
 * @see Counter.ts https://gitee.com/ifaswind/eazax-ccc/blob/master/components/Counter.ts
 * @version 20210521
 */
@ccclass
@requireComponent(cc.ProgressBar)
export default class CounterProgressBar extends cc.Component {

    @property(cc.Label)
    public progressBar: cc.ProgressBar = null;

    @property({ tooltip: CC_DEV && '动画时长' })
    public duration: number = 0.5;

    @property({ tooltip: CC_DEV && '保持数值为整数' })
    public keepInteger: boolean = true;

    protected actualProgress: number = 0;

    public get progress() {
        return this.actualProgress;
    }

    public set progress(value: number) {
        if (this.keepInteger) {
            value = Math.floor(value);
        } else {
            value = parseFloat(value.toFixed(1))
        }
        this.curProgress = (this.actualProgress = value);
    }

    protected _curProgress: number = 0;

    public get curProgress() {
        return this._curProgress;
    }

    public set curProgress(value: number) {
        if (this.keepInteger) {
            value = Math.floor(value);
            this._curProgress = value;
            // this.label.string = value.toString();
            this.progressBar.progress = value;
            return;
        }
        this._curProgress = value;
        // this.label.string = this.formatterHundred(value);
        this.progressBar.progress = parseFloat(this.formatterHundred(value));
    }

    protected tweenRes: Function = null;

    protected onLoad() {
        this.init();
    }

    /**
     * 初始化
     */
    protected init() {
        if (!this.progressBar) {
            this.progressBar = this.getComponent(cc.ProgressBar);
        }
        this.curProgress = 0;
    }

    /**
     * 设置数值
     * @param value 数值
     */
    public set(value: number) {
        this.curProgress = value;
    }

    /**
     * 设置动画时长
     * @param duration 动画时长
     */
    public setDuration(duration: number) {
        this.duration = duration;
    }

    /**
     * 滚动数值
     * @param value 目标值
     * @param duration 动画时长
     * @param callback 完成回调
     */
    public to(value: number, duration?: number, callback?: () => void): Promise<void> {
        return new Promise<void>(res => {
            // 停止当前动画
            if (this.tweenRes) {
                cc.Tween.stopAllByTarget(this);
                this.tweenRes();
            }
            this.tweenRes = res;
            // 保存实际值
            this.actualProgress = value;
            // 动画时长
            if (duration == undefined) {
                duration = this.duration;
            }
            // GO
            cc.tween<CounterProgressBar>(this)
                .to(duration, { curProgress: value })
                .call(() => {
                    this.tweenRes = null;
                    callback && callback();
                    res();
                })
                .start();
        });
    }

    /**
     * 相对滚动数值
     * @param diff 差值
     * @param duration 动画时长
     * @param callback 完成回调
     */
    public by(diff: number, duration?: number, callback?: () => void): Promise<void> {
        const value = this.actualProgress + diff;
        return this.to(value, duration, callback);
    }

    formatterHundred(value: number): string {
        if (value === 0 || value === 100) {
            return value.toString()
        } else {
            return value.toFixed(1)
        }

    }

}
