...
export default class ResAreaControl extends cc.Component {
  ...
  onTouchStart({ event, res }: { event: cc.Event.EventTouch; res: ResControl }) {
    ...
    const _curResNode = res.unique ? res.node : this.getEffectiveResNode(res.tags);
    if (_curResNode) {
  ...
  getEffectiveResNode(tags: number[]) {
    ...
    const _curResNode = copyArray.reverse().find(resNode => {
      return cc.isValid(resNode) && resNode.active && gi.Utils.hasIntersection(resNode.getComponent(ResControl).tags, tags);
    });
    return _curResNode;
  }
  ...
  destroyCurRes() {
    if (cc.isValid(this.curRes.node)) {
      const index = this.children.findIndex(value => this.curRes.node === value);
      if (index !== -1) {
        this.children.splice(index, 1);
      }
  ...
  cancleCurRes() {
    if (cc.isValid(this.curRes.node)) {
      ...
      const worldPos = this.curRes.node.convertToWorldSpaceAR(cc.v2(0, 0));
      this.curRes.node.setPosition(this.curResOriParent.convertToNodeSpaceAR(worldPos));
