...
export default class GuideManager extends cc.Component {
  ...
  showGuideText(guide: GuideControl) {
    ...
    const guideInfo = gi.getLevelInfo().guideMap.find(item => item.key === guide.key);
  ...
  run() {
    ...
    for (const guideNode of guideNodes) {
      ...
      if (guide.type !== gi.Guide.Type.Text && !guide.targetNode.active) continue;
  ...
  drag(guide: GuideControl) {
    ...
    this.guideIcon.setPosition(this.guideIcon.parent.convertToNodeSpaceAR(resNode.convertToWorldSpaceAR(cc.v2(0, 0))));
    // 克隆附带节点
    // const copyNode = cc.instantiate(resNode);

    this.guideIcon.active = true;

    gi.Guide.fromToNode(resNode, guide.targetNode, { time: 2, guide: this.guideIcon }).union().repeatForever().start();
  ...
  click(guide: GuideControl) {
    ...
    // 停止之前的缓动并设置位置
    this.guideIcon.stopAllActions();
    this.guideIcon.setPosition(this.guideIcon.parent.convertToNodeSpaceAR(guide.targetNode.convertToWorldSpaceAR(cc.v2(0, 0))));
  ...
  swipe(guide: GuideControl) {
    ...
    gi.Guide.fromToPos(this.guideIcon.getPosition(), targetPos, { time: 1, guide: this.guideIcon }).union().repeatForever().start();
  }
  ...
  getEffectiveTrigger(res: ResControl) {
    ...
    while (limitedCount) {
      const randomIndex = Math.floor(Math.random() * this.triggerManager.children.length);
      const trigger = this.triggerManager.children[randomIndex].getComponent(TriggerControl);
      if (trigger.node.active && trigger.key && trigger.canTriggerOff(res.tags)) return trigger;
      limitedCount--;
    }
  }
}
