...
export function chicken(options: gi.SubscriptionOptions) {
  options.target.on(cc.Node.EventType.TOUCH_END, () => {
    ...
    (cc.tween(options.nodes[0]) as cc.Tween).by(0.1, { angle: -8 }).by(0.1, { angle: 8 }).union().repeatForever().start();
    (cc.tween(options.nodes[1]) as cc.Tween).by(0.1, { angle: 8 }).by(0.1, { angle: -8 }).union().repeatForever().start();
...
export function phone(options: gi.SubscriptionOptions) {
  options.target.on(cc.Node.EventType.TOUCH_END, () => {
    ...
    (cc.tween(options.nodes[0]) as cc.Tween).by(0.1, { angle: -8 }).by(0.1, { angle: 8 }).union().repeatForever().start();
    (cc.tween(options.nodes[1]) as cc.Tween).by(0.1, { angle: -8 }).by(0.1, { angle: 8 }).union().repeatForever().start();
    (cc.tween(options.nodes[2]) as cc.Tween).by(0.1, { angle: 8 }).by(0.1, { angle: -8 }).union().repeatForever().start();
