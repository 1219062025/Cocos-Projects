...
export default class QuadTree<T> {
  ...
  Search(x: number, y: number) {
    ...
    while (!CurrentTree.isLeaf()) {
      ...
      if (x > TopRightTree.LeftX && y > TopRightTree.BottomY) {
        // 在第一象限子树内
        CurrentTree = TopRightTree;
      } else if (x < TopLeftTree.RightX && y > TopLeftTree.BottomY) {
        // 在第二象限子树内
        CurrentTree = TopLeftTree;
      } else if (x < BottomLeftTree.RightX && y < BottomLeftTree.TopY) {
        // 在第三象限子树内
        CurrentTree = BottomLeftTree;
      } else if (x > BottomRightTree.LeftX && y < BottomRightTree.TopY) {
        // 在第四象限子树内
        CurrentTree = BottomRightTree;
      } else {
        break;
